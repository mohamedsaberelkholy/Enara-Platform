...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onNavigate, students, setStudents, onViewStudent }) => {
  ...
  return (
    <div className="space-y-6 sm:space-y-8 p-4 sm:p-8 max-w-[1600px] mx-auto">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {stats.map((stat, i) => {
            ...
            return (
              <div key={i} className="bg-white p-5 sm:p-6 rounded-2xl shadow-sm border border-slate-100 card-hover">
                <div className="flex justify-between items-start mb-3 sm:mb-4">
                  <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl ${stat.bg} ${stat.color} flex items-center justify-center`}>
                    <Icon size={20} className="sm:w-6 sm:h-6" />
                  </div>
                  <div className={`flex items-center gap-0.5 px-2 py-1 rounded-lg text-[10px] sm:text-xs font-bold ${stat.trendUp ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600'}`}>
                ...
                <div>
                  ...
                  <h3 className="text-2xl sm:text-3xl font-bold text-slate-900 mt-1">{stat.value}</h3>
                  <p className="text-[10px] sm:text-xs text-slate-400 mt-2">vs. previous period</p>
                </div>
              </div>
            );
          })}
      ...
      <section>
        <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
          <BarChart data={[]} className="hidden" /> {/* Dummy to keep imports clean if needed, but I'll use a lucide icon */}
          <Activity size={20} className="text-indigo-600" />
          Academic Insights & Engagement
        </h2>
      ...
      <section>
        <h2 className="text-xl font-bold text-slate-900 mb-6 flex items-center gap-2">
          ...
          Student Roster & Detailed Progress
        </h2>
          ...
          <div className="p-4 sm:p-6 border-b border-slate-100 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
              ...
              <button 
                ...
                className="px-4 py-2 rounded-xl teal-gradient text-white text-sm font-bold shadow-lg shadow-teal-900/10 hover:scale-[1.02] active:scale-95 transition-all"
              >
                Add Student
              </button>
            </div>
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full teal-gradient flex items-center justify-center text-white font-bold text-xs shrink-0">
                          {s.avatar}
                        </div>
